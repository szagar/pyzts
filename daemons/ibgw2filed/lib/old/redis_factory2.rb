...
class RedisFactory2
  ...
  def initialize
    show_info "RedisFactory2#initialize"
  ...
  def client(ctype="std")
    show_info "RedisFactory2#client(#{ctype})"
    if ctype == "mkt_data"
...
show_info "@redis=#{@redis.class}"
    @redis ||= connect(url, :timeout => 0.7)
  end
...
=end
    ...
    show_info "Redis.connect(#{options})"

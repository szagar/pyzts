...
class ConfirmsCache # < RedisStore
  include Store
  include LogHelper
  ...
  def action_BUY(account,act)
    write(account, act[:tdate], act[:pid], act)
  ...
  def pk(account,tdate,pid)
    "confirms:#{account}:#{tdate}:#{pid}"
  end

  def write(account_name, date, pid, params)
    # XACT:deposit 20140519 100000 to p_exit_tightest, note:
    #action "XACT:#{xtype} #{date} #{amount} to #{account_name}, note: #{note}"
    redis.sadd pk(account_name,date,pid),params

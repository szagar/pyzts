...
class IbDaytrader
  ...
  def initialize(broker,opts={})
    show_info "IbDaytrader#initialize"
  ...
  def command_received()
    # submit_trade(to_broker,payload)
    # md_unrequests(sec_id)
  end

  def submit_trade(to_broker,payload)
  ...
  def md_requests(sec_id,mkt,action,force)
      debug "md_requests(#{sec_id},#{mkt},#{action},#{force})"
      contract = get_contract( mkt, sec_id )

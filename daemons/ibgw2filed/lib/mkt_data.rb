...
class MktData
  ...
  def subscribe_entry
    ...
    @channel.queue("", auto_delete: true)
      ...
      params  = msg[:params]
      show_info "MktData#subscribe_entry: acct    = #{acct}"
      show_info "MktData#subscribe_entry: command = #{command}"
      show_info "MktData#subscribe_entry: params  = #{params}"
  ...
  def subscribe_lvc_data
    ...
    show_info "MktData#subscribe_lvc_data: subscribe:(#{exchange_market.name}/#{routing_key})"
  ...
  def subscribe_md_bar5s
    ...
    show_info "MktData#subscribe_md_bar5s: subscribe:(#{exchange_market.name}/#{routing_key})"

    md_channel.queue("", auto_delete: true)
           .bind(exchange_market, routing_key: routing_key)
           .subscribe do |hdr,msg|
      print "*"
      show_info "MktData#subscribe_md_bar5s: #{msg}"
  ...
  def sod_process(parms={})
    show_action "MktData:sod_process"
    #portf_mgr.sids_of_open_positions.each do |sid|
    #  show_action "MktData#sod_process: request_md(#{sid})"
    #  request_md(sid)
    #end
  end

  def eod_process(parms={})
    show_action "MktData#eod_process DO NOTHING"
  end

  def load_watchlists(parms={})
    show_action "MktData:load_watchlists"
    @do_not_exit_list  = load_do_not_exit_list
    show_exit_watchlist
    @do_not_enter_list = load_do_not_enter_list
  ...
  def submit_setups(parms={})
    show_action "MktData#submit_setups DO NOTHING"
  end

  def submit_daytrade(account_name,entry)
    show_action "MktData:submit_daytrade(#{entry})"
    debug "MktData#submit_daytrade limit_price=#{entry.limit_price}"
  ...
  def trailing_stops(parms={})
    show_action "MktData#trailing_stops DO NOTHING"
  end
  ...
  def scale_in?
    warn "MktData: scale_in? needs some code here"
    false
  end
...
=end
  ...
  def request_md(sid)
    show_action "MktData#request_md(#{sid})"
    show_info "MktData#request_md: subscribe sid(#{sid})"
  ...
  def check_for_exits(bar)
    debug "MktData#check_for_exits(#{bar})"
  ...
  def submit_order(order)
    debug "MktData#submit_order(#{order})"
  ...
  def check_auto_traders(bar)
    at_mgr.check_for_entries(bar).each do |entry_id|
      show_action "MktData#check_auto_traders: entry_id=#{entry_id}"
      trade(entry_id)
    end
  end
